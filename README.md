...
> **후속 안전 개선:** WhatsApp 명령 제한, 썸네일, 백업 v3, Compose 버전 고정 및 임시글 사본에 대한 로컬 변경은 별도 검증·운영 배포 기록을 확인해야 합니다. 이 저장소의 새 코드가 이미 운영 서버에 적용됐다고 간주하지 마세요.
...
Compose의 WordPress 및 MariaDB 이미지는 2026-09-20 운영 서버에서 관측한 Linux amd64 이미지 digest로 고정되어 있습니다. 기본 포트는 운영의 HTTPS 역방향 프록시 구성과 같이 호스트 내부 `127.0.0.1:8080`에만 바인딩됩니다. 새 호스트에서는 역방향 프록시와 플랫폼 호환성을 별도로 설정·검증해야 합니다. 이미 실행 중인 운영 컨테이너를 이 설정으로 재생성하기 전 실제 DB 환경변수와 복원 가능성을 확인하세요.
...
기존 **v2 운영 백업**은 DB·미디어·일부 설정 등 3개 구성요소를 보관합니다. **로컬에서 개선한 v3 스크립트**는 다음 구성요소를 포함하되, 운영 서버에 배포하고 실제 백업·격리 복원을 마치기 전에는 완전 복구를 보장하지 않습니다.

1. `db.sql.gz`: MariaDB 덤프.
2. `uploads.tar.gz`: 업로드 미디어.
3. `wp-content.tar.gz`: 플러그인·테마·선택적으로 MU 플러그인.
4. `configs.tar.gz`: Docker·비밀정보가 아닌 설정과 런타임 JSON.
5. `secrets.tar.gz`: 환경 설정·WordPress 인증 정보 등 별도 구성요소. **파일 권한으로만 보호되는 아카이브 내부의 평문이므로 저장소 외부의 암호화·접근 통제 정책이 추가로 필요합니다.**
6. `manifest.json`: 구성요소의 크기·SHA256.

아카이브는 임시 위치에서 생성된 뒤 최종 이름으로 이동하며 기존 운영본의 7일 보관 정책을 유지합니다. Nginx 설정은 검토된 별도 디렉터리를 `NGINX_CONFIG_DIR`로 명시한 경우에만 백업되며, TLS 키의 전체 복원은 별도 운영 절차가 필요합니다.
...
### 2) 체크섬 검증 및 격리 복구
```bash
# 운영 데이터 변경 없이 구성요소 해시와 경로 검사
./agent-publisher/restore_backup.sh /path/to/snapshot.tar.gz --verify-only

# 새로운 격리 디렉터리에 설정을 추출, 자동 설치하거나 운영 설정을 덮어쓰지 않음
./agent-publisher/restore_backup.sh /path/to/snapshot.tar.gz --export-configs /new/private/export
```
실제 데이터베이스·WordPress 파일 복원은 검증을 마친 **격리된 테스트 환경에서만** 대화형으로 실행하세요. `--yes`는 현재 지정된 Docker 컨테이너의 DB·파일을 교체하므로 운영 서버에서 무심코 실행해서는 안 됩니다. 구형 v2 백업은 `--verify-only`가 가능하더라도 플러그인·테마 누락으로 전체 복원이 거부됩니다. 비밀정보는 별도 `--export-configs ... --include-secrets`로 제한된 새 디렉터리에만 추출하며 자동 설치하지 않습니다.
