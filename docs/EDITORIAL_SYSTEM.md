...
렌더러는 첫 문단을 ‘핵심요약’으로 강조하고 본문 소제목에 번호를 붙인다. 신청·수령·연장처럼 순서가 있는 주제는 sections를 실제 단계 순서로 설계하고, 준비물·조건처럼 병렬 정보는 체크리스트나 표로 설계한다. 출처 링크는 문단마다 반복하지 않고 글 하단의 ‘공식 출처’ 목록에 한 번만 표시한다.
