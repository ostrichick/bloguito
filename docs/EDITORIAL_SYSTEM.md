...
렌더러는 첫 문단을 '3초 핵심요약' 카드로 강조하고 본문 소제목에 단계별 번호(STEP)를 붙인다. 신청·수령·연장처럼 순서가 있는 주제는 sections를 실제 단계 순서로 설계하고, 준비물·조건처럼 병렬 정보는 체크리스트나 표로 설계한다. FAQ는 Q&A 분리형 카드로 렌더링한다. 출처 링크는 문단마다 반복하지 않고 글 하단의 '공식 출처' 목록에 기관 검증 배지와 함께 한 번만 표시한다.
