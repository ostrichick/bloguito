...
내부 기간 검사 결과(예: 오늘은 신청기간 안에 해당)를 독자용 본문에 넣지 마세요.
자료 확인일과 포괄적인 면책문(지급·선정·접수 가능 여부를 보장하지 않습니다 등)은 본문 말미에 넣지 마세요.
확인일은 내부 출처 기록에 보관하고, 실제 신청에 영향을 주는 구체적인 자격·제외 조건은 관련 설명에 명확히 쓰세요.
FAQ를 작성하는 형식에서는 질문에 대한 지급 방식·날짜·조건을 답변 자체에 명시해야 합니다.
이미 본문에 있어도 답에 필요한 사실은 반복하고, '위 표/공식 안내를 확인하세요'로 답을 대체하지 마세요.
